...
public class Program
{
	static void Main(string[] args)
	{
		...
		{
			new Debtor("Shirley T. Qua18simvolllls", DateTime.Parse("March 30, 1932"), "123-456-7890", "ShirleyTQualls@teleworm.us", "3565 Eagles Nest Drive Woodland, CA 95695", 2414),
			new Debtor("Danielle W. Grrrier", DateTime.Parse("October 18, 1953"), "321-473-4178", "DanielleWGrier@rhyta.com", "1973 Stoneybrook Road Maitland, FL 32751", 3599),
			...
			new Debtor("Coy K. Adams", DateTime.Parse("March 1, 1976"), "410-344-1307", "CoyKAdams@dayrep.com", "2411 Blue Spruce Lane Baltimore, MD 21202", 3784),
			new Debtor("Bernice J. Miles", DateTime.Parse("June 1, 1989"), "912-307-6797", "BerniceJMiles@armyspy.com", "4971 Austin Avenue Savannah, GA 31401", 600),
			...
			new Debtor("Adam D. Bar18simvoltlett", DateTime.Parse("May 6, 1950"), "657-673-7778", "AdamDBartlett@rhyta.com", "2737 Rardin Drive San Jose, CA 95110", 5412),
			...
			new Debtor("Au18simdvorly M. Styles", DateTime.Parse("February 7, 1937"), "978-773-4772", "AudryMStyles@jourrapide.com", "151 Christie Way Marlboro, MA 01752", 34500),
			...
			new Debtor("Doris B. King", DateTime.Parse("October 10, 1978"), "205-231-8973", "DorisBKing@rhyta.com", "3172 Bedford Street Birmingham, AL 35203", 2589),
			...
			new Debtor("Carla A. Creagh", DateTime.Parse("November 21, 1990"), "614-307-8974", "CarlaACreagh@dayrep.com", "3106 Bates Brothers Road Columbus, OH 43215", 11271),
			new Debtor("Norma M. New", DateTime.Parse("May 18, 1988"), "142-356-7098", "NormaMNew@jourrapide.com", "965 Metz Lane Woburn, MA 01801", 9761),
			...
			new Debtor("Dave N. Rodriguez", DateTime.Parse("January 21, 1938"), "321-978-4506", "DaveNRodriguez@rhyta.com", "1890 Poco Mas Drive Dallas, TX 75247", 9132),
		...
		//debtors.ForEach(
		//(de) => { if (de.Email.EndsWith("rhyta.com") || de.Email.EndsWith("dayrep.com")) Console.WriteLine(de); }
		//);
		...
		//debtors.ForEach(
		//(de) => { if (year - de.BirthDay.Year >= 26 && year - de.BirthDay.Year <= 36) Console.WriteLine(de); }
		//);
		...
		//debtors.ForEach(
		//(de) => { if (de.Debt <= 5000) Console.WriteLine(de); }
		//);
		...
		//debtors.ForEach(
		//(de) => { if (de.FullName.Length >= 18 && de.Phone.Count(ch => ch=='7') >= 2) Console.WriteLine(de); }
		//);
		...
		//debtors.ForEach(
		//(de) => { if (de.BirthDay.Month > 11 || de.BirthDay.Month < 3) Console.WriteLine(de); }
		//);
		...
		/*  8)  Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sort etmek  */

		//var avgDebt = debtors.Average(de => de.Debt);

		//var debtors2 = debtors.Where(de => de.Debt > avgDebt).ToList();

		//debtors2.Sort((d1, d2) => (d1.FullName.CompareTo(d2.FullName)));
		//debtors2.Sort((d1, d2) => (d1.Debt.CompareTo(d2.Debt)));

		//debtors2.ForEach(de => Console.WriteLine(de));
		...
		//var debtors3 = debtors.Where(de => de.Phone.Contains('8') == false).ToList();

		//foreach (var de in debtors3)
		//	Console.WriteLine($"Name : {de.FullName} | Age : {2024 - de.BirthDay.Year} | Debt : {de.Debt} AZN");
		...
		/*  11)  Adinda ve familyasinda hec olmasa 3 eyni herf olan borclularin siyahisin cixarmaq ve onlari elifba sirasina gore sort elemek  */

		//var debtors2 = debtors.Where(d => d.FullName.ToLower().GroupBy(c => c)
		//	.ToDictionary(g => g.Key, g => g.Count())
		//	.Any(pair => pair.Value >= 3)).ToList();

		//debtors2 = debtors2.OrderBy(de => de.FullName).ToList();
		...
		//var year = debtors.GroupBy(de => de.BirthDay.Year).OrderByDescending(gr => gr.Count()).First().Key;
		...
		//for (int i = 0; i<5; i++) Console.WriteLine(debtors[i]);
		...
		//debtors.ForEach(de => totalDebt += de.Debt);

		//      Console.WriteLine($"\n\n\tTotal Debt : {totalDebt}  AZN.\n");


		////       way 2     ---------------

		//totalDebt = debtors.Sum(de => de.Debt);
		...
		//debtors.ForEach(
		//		de => { if (de.BirthDay.Year < 1945) { Console.WriteLine(de); } }
		//);
		...
		//debtors.ForEach(de => {
		//		if ((de.Phone.Distinct().Count()) == (de.Phone.Length-1))
		...
		//debtors.ForEach(de => { 
		//		if (de.Debt <= (Math.Abs(DateTime.Now.Month-de.BirthDay.Month))*500) 
		//			Console.WriteLine(de);
		//	});
		...
		//debtors.ForEach(de =>{ if (de.FullName.Contains('s') && de.FullName.Contains('m')
		//	&& de.FullName.Contains('i')&& de.FullName.Contains('l') && de.FullName.Contains('e')) 
		//		{ Console.WriteLine(de); } } );
